...
void nearLAZA( int i, int j, unsigned char t_image[][Max_Dimension] )
{						
	if ( ( i % 2 ) && ( j % 2 ) ) // i odd, j odd, center pixel
	{						
		t_image[i][j] = Min( ( t_image[i + 1][j + 1] + t_image[i - 1][j - 1] ) / 2, ( t_image[i + 1][j - 1] + t_image[i - 1][j + 1] ) / 2 );										
	}
	else if ( !( i % 2 ) && ( j % 2 ) ) // i even j odd, horizontal pixel
	{		
		t_image[i][j] = ( t_image[i][j - 1] + t_image[i][j + 1] ) / 2;				
	}
	else if ( ( i % 2 ) && !( j % 2 ) ) // i odd j even, vertical pixel
	{		
		t_image[i][j] = ( t_image[i + 1][j] + t_image[i - 1][j] ) / 2;						
	}	
	else
	{
		// i even j even = The original image pixels
...
void Stage_1( int i, int j, unsigned char t_image[][Max_Dimension] )
{						
	if ( ( i % 2 ) && ( j % 2 ) ) // i odd, j odd, center pixel
	{			
		unsigned char a = t_image[i - 1][j - 1];
		unsigned char b = t_image[i - 1][j + 1];
		unsigned char c = t_image[i + 1][j - 1];
		unsigned char d = t_image[i + 1][j + 1];

		unsigned char m1 = Max( Max( a, b ), Max( c, d ) );								
		unsigned char m2 = Min( Min( a, b ), Min( c, d ) );								
		

		if ( AbsDif( m1, m2 ) < 4 )
		{
			t_image[i][j] = ( a + b + c + d ) / 4;								
		}
...
void Image_Magnification()
{
	...
	for ( i = 0; i < M * H  - 1; ++i )
	{
		for ( j = 0; j < M * W - 1; ++j )
		{
			...
			nearLAZA( i, j, r_image );			
			nearLAZA( i, j, g_image );			
			nearLAZA( i, j, b_image );	
...
void main()
{
	...
	strcpy( i_file, ".\\Image\\1.bmp");
	strcpy( o_file, ".\\Image\\x2laza.bmp");
	
	Image_Input(); 				
	Image_Magnification();		
	Image_Output();
		
	strcpy( i_file, ".\\Image\\x2laza.bmp");
	strcpy( o_file, ".\\Image\\x4laza.bmp");
